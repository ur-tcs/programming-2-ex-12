...
Actual compression tools are not based on Huffman code alone, as this would not result in very good compression rates. The popular [bzip2](https://en.wikipedia.org/wiki/Bzip2) tool, for instance, applies several stages of compression, one of which are Huffman codes. Another stage used in bzip2 is the [Burrows-Wheeler transform](https://en.wikipedia.org/wiki/Burrows%E2%80%93Wheeler_transform), which is a remarkably elegant (and invertible) way of permuting the characters in a string, in such a way that the Huffman encoding becomes much more effective.
